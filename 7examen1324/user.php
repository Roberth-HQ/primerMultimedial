...
$sql_persona = "SELECT * FROM persona WHERE ci = $ci";
$resultado_persona = mysqli_query($conn, $sql_persona);
$fila = mysqli_fetch_assoc($resultado_persona);
$nombre = $fila["nombre"];
$apellido = $fila["paterno"];

$sql_catastro = "SELECT * FROM catastro WHERE ci = ?";
if ($stmt = mysqli_prepare($conn, $sql_catastro)) {
    mysqli_stmt_bind_param($stmt, "s", $ci);
    mysqli_stmt_execute($stmt);
    $resultado_catastro = mysqli_stmt_get_result($stmt);
...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    <li class="nav-item">
                        <a class="nav-link" href="#">Bienvenido, <?php echo "$nombre $apellido ($ci)"; ?>:</a>
                    </li>
                </ul>
                <div class="d-flex">
    ...
    <div class="custom-container">
        <table class="table table-striped custom-table">
            ...
            <tbody>
                <?php
                while ($fila = mysqli_fetch_array($resultado_catastro)) {
                    echo "<tr>";
                    echo "<td>$fila[ci]</td>";
                    echo "<td>$fila[id]</td>";
                    ...
                    echo "<td>$fila[cod]</td>";
                    echo "<td>";
                    echo "<button class='btn btn-outline-light' onclick=\"window.location.href='http://localhost:8090/mavenproject3/examen?cod=$fila[cod]';\">Control de Pago en C#</button>";
                    echo "<button class='btn btn-outline-warning' onclick=\"window.location.href='http://localhost:8090/mavenproject3/examen?cod=$fila[cod]';\">Control de Pago en JAVA</button>";
                    echo "</td>";
                    echo "</tr>";
                }
                ?>
    ...
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9Ghb2y4SYWlb/NWsDX7B" crossorigin="anonymous"></script>
</body>
</html>
