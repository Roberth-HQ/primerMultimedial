...
$sqlD = "SELECT numero, zona FROM distrito"; 
$resultadoD = mysqli_query($conn, $sqlD);

if (!$resultadoD) {
    $error_message = "Error al obtener los distritos: " . mysqli_error($conn);
}
?>
...
<body>
    ...
    <div class="form-container">
        ...
        <?php endif; ?>
        <form action="process_ajax.php?ci=<?php echo urlencode($ci); ?>&distrito=<?php echo urlencode($distrito); ?>" method="POST">
            <div class="form-row">
            ...
            <div class="form-row">
                ...
                <div class="form-column">
                <label for="distrito" class="form-label">Distrito: <?php echo htmlspecialchars($distrito); ?></label>
                <select name="zona" id="zona" required>
                    <option value="">Selecciona una Zona</option>
                    <?php 
                        while($filaD = mysqli_fetch_array($resultadoD)) {
                            if ($distrito == $filaD["numero"]) {
                                echo '<option value="'.$filaD["zona"].'">'.$filaD["zona"].'</option>';
                            }
                        }
                    ?>
                </select>
            ...
            <input type="hidden" name="distrito" value="<?php echo htmlspecialchars($distrito); ?>">
