...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $ci = $_POST["ci"]; // Se mantiene el CI del formulario
    $distrito = $_POST["distrito"];
    $cod = $_POST["cod"];

    // Prepara la consulta SQL para insertar el nuevo registro en el catastro
    $sql = "INSERT INTO catastro (zona, xini, yini, xfin, yfin, superficie, ci, distrito, cod) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    // Prepara la declaración
    if ($stmt = mysqli_prepare($conn, $sql)) {
        // Vincula los parámetros (s para string, d para double, i para integer)
...
// Carga el CI desde la URL
$ci = isset($_GET['ci']) ? $_GET['ci'] : ''; // Obtiene el CI de la URL
?>
